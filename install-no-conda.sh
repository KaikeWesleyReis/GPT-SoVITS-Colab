...
run_apt_quiet() {
    local output
    output=$(apt-get install -y -q "$@" 2>&1) || {
...
if [ "$(uname)" != "Darwin" ]; then
    ...
    if [ "$gcc_major_version" -lt 11 ]; then
        ...
        echo -e "${INFO}Installing libstdc++ dev from apt"
        run_apt_quiet "libstdc++-${gcc_major_version}-dev"
        echo -e "${SUCCESS}libstdc++-${gcc_major_version}-dev Installed..."
    fi
else
...
run_apt_quiet ffmpeg cmake make
...
if python -c "import torch" &>/dev/null 2>&1; then
    echo -e "${INFO}PyTorch already installed, skipping PyTorch installation"
elif [ "$USE_CUDA" = true ] && [ "$WORKFLOW" = false ]; then
    if [ "$CUDA" = 128 ]; then
